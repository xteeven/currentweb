...
const getInfo = (app, rootDir, base = "") => {
    ...
    const result = utils_1.fs
        ...
        const stat = utils_1.fs.statSync(fileFullPath);
        ...
        if (stat.isDirectory()) {
            ...
            return {
                ...
                index: "index" in dirInfo ? dirInfo.index : null,
        ...
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        const page = pages.find((page) => page.filePathRelative === utils_1.path.join(rootDir, fileRelativePath));
        return {
            type: "file",
            path: filename,
            title: page.title,
        ...
        .filter((item) => item !== null && item.index !== false)
        // sort items
        .sort((itemA, itemB) => {
...
const getGeneratePaths = (sidebarConfig) => {
    const result = [];
    sidebarConfig.forEach((item) => {
        ...
        if (typeof item === "object" && "children" in item) {
            ...
            if (item.children === "structure")
                ...
                result.push(...getGeneratePaths(item.children));
        }
    });
    return result;
};
